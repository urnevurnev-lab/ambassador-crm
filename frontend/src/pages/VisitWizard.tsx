...
const VisitWizard: React.FC = () => {
  ...
  const scenarioSteps = useMemo(() => {
    switch (visitType) {
      case 'transit':
        return ['Что стоит в заведении', 'Отчет'];
      case 'checkup':
        return ['Смена', 'Отчет'];
      case 'tasting':
    ...
    try {
      ...
      if (visitType === 'transit') {
        ...
        if (Object.keys(cleaned).length > 0) {
          scenarioData.sampleUsage = cleaned;
        }
      }
  ...
  return (
    <div className="space-y-6 pb-24">
      ...
      {(currentStep === 'Что стоит в заведении' || currentStep === 'Чек-ин') && (
          ...
          {visitType === 'transit' && (
            <div className="space-y-4">
              <div className="bg-white rounded-3xl border border-gray-100 shadow-sm p-5">
                <div className="flex items-center justify-between">
                  <div>
                    <div className="text-xs font-semibold uppercase tracking-widest text-gray-500">Что курили</div>
                    <p className="text-sm text-gray-500 mt-1">Отметьте вкусы (спишется с баланса пробников)</p>
                  </div>
                  <div className="px-3 py-1 rounded-2xl bg-black/5 text-[12px] font-semibold text-black/60 border border-white/40">
                    {sampleUsageTotal} шт.
                  </div>
                </div>

                <div className="flex gap-2 mt-3 overflow-x-auto no-scrollbar">
                  {Object.keys(groupedProducts).map((line) => (
                    <button
                      key={line}
                      onClick={() => setSampleUsageLine(line)}
                      className={`px-4 py-2 rounded-2xl text-[12px] font-semibold border ${
                        sampleUsageLine === line
                          ? 'bg-black text-white border-black shadow'
                          : 'bg-[#F5F5F7] text-gray-700 border-white'
                      }`}
                    >
                      {line}
                    </button>
                ...
                <div className="relative mt-3">
                  ...
                  <input
                    ...
                    onChange={(e) => setSampleUsageSearch(e.target.value)}
                    placeholder="Поиск по вкусу"
                    className="w-full bg-[#F5F5F7] rounded-2xl pl-10 pr-3 py-3 text-sm font-medium border border-gray-100 focus:bg-white focus:ring-2 focus:ring-black/5 outline-none"
                  />
                </div>

                <div className="mt-4 space-y-2 max-h-[220px] overflow-y-auto pr-1">
                  {(groupedProducts[sampleUsageLine] || []).filter((p) =>
                    p.flavor.toLowerCase().includes(sampleUsageSearch.toLowerCase())
                  ).map((p) => {
                    const count = sampleUsage[p.id] || 0;
                    return (
                      <div
                        key={p.id}
                        className="flex items-center justify-between bg-[#F5F5F7] rounded-2xl px-3 py-2 border border-white"
                      >
                        <div className="text-sm font-semibold text-gray-800">{p.flavor}</div>
                        <div className="flex items-center gap-2">
                          <button
                            onClick={() =>
                              setSampleUsage((prev) => {
                                const next = { ...prev };
                                const nextVal = (next[p.id] || 0) - 1;
                                if (nextVal <= 0) delete next[p.id];
                                else next[p.id] = nextVal;
                                return next;
                              })
                            }
                            className="w-8 h-8 rounded-full bg-white border border-gray-200 text-gray-500 flex items-center justify-center active:scale-95"
                          >
                            -
                          </button>
                          <div className="w-8 text-center text-sm font-semibold">{count}</div>
                          <button
                            onClick={() =>
                              setSampleUsage((prev) => ({ ...prev, [p.id]: (prev[p.id] || 0) + 1 }))
                            }
                            className="w-8 h-8 rounded-full bg-black text-white flex items-center justify-center active:scale-95"
                          >
                            +
                          </button>
                        </div>
            ...
            <div className="mt-3 text-sm text-gray-600 space-y-2">
              ...
              {visitType === 'transit' && (
                <>
                  ...
                  <div>Что курили (шт): {sampleUsageTotal}</div>
                </>
              )}
              {visitType === 'checkup' && (
                <>
